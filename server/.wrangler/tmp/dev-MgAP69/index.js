...
// .wrangler/tmp/bundle-BhdYGt/checked-fetch.js
...
// src/generateAI.ts
var PROMPT_PREFIX = `Write a detailed analysis on these factors to achieve desired goals for a person based on their gender living in India, define suitable workout split and diet plan in proper points and styling in html tags format `;
async function groqGenerate(apiKey, clientData) {
  const groq = new groq_sdk_default({ apiKey });
...
async function handleChat(apiKey, message) {
  const groq = new groq_sdk_default({ apiKey });
  const chatCompletion = await groq.chat.completions.create({
    messages: [
      {
...
async function generateRecommendations(clientData, model, apiKey) {
  if (model === "groq") {
...
async function handleChatMessage(apiKey, message) {
  const isRelatedToGym = /gym|exercise|workout|health|diet|nutrition|hi|hello/i.test(message);
  if (!isRelatedToGym) {
...
// node_modules/@hono-rate-limiter/cloudflare/index.esm.js
var t = class {
  constructor(e) {
  ...
  async get(e) {
    const t2 = await this.namespace.get(this.prefixKey(e), "json");
    if (t2)
      return t2;
  }
  async increment(e) {
    const t2 = this.prefixKey(e);
    let s = await this.namespace.get(t2, "json");
    return s ? s.totalHits += 1 : (s = { totalHits: 1, resetTime: /* @__PURE__ */ new Date() }, s.resetTime.setTime(this.windowMs)), await this.namespace.put(t2, JSON.stringify(s), { expiration: Math.floor(s.resetTime.getTime() / 1e3) }), s;
  }
  async decrement(e) {
    const t2 = this.prefixKey(e), s = await this.namespace.get(t2, "json");
    s && (s.totalHits -= 1, await this.namespace.put(t2, JSON.stringify(s), { expiration: Math.floor(s.resetTime.getTime() / 1e3) }));
  }
  async resetKey(e) {
...
// node_modules/hono-rate-limiter/index.esm.js
...
var setDraft6Headers = (context, info, windowMs) => {
  ...
  if (resetSeconds)
    context.header("RateLimit-Reset", resetSeconds.toString());
};
...
function rateLimiter(config) {
  ...
  const options = { windowMs, limit, message, statusCode, standardHeaders, requestPropertyName, requestStorePropertyName, skipFailedRequests, skipSuccessfulRequests, keyGenerator, skip, requestWasSuccessful, handler, store };
  ...
  return createMiddleware(async (c, next) => {
    ...
    const info = { limit: _limit, used: totalHits, remaining: Math.max(_limit - totalHits, 0), resetTime };
    c.set(requestPropertyName, info);
    c.set(requestStorePropertyName, { getKey: store.get?.bind(store), resetKey: store.resetKey.bind(store) });
...
app.use(
  (c, next) => rateLimiter({
    ...
    keyGenerator: (c2) => c2.req.header("cf-connecting-ip") ?? "",
    // Method to generate custom identifiers for clients.
    store: new t({ namespace: c.env.CACHE })
...
app.post("/gen", async (c) => {
  try {
    ...
    if (!age || !experience || !schedule || !hours || !goal || !diet || !height || !weight || !gender) {
      return c.status(400);
    }
...
// .wrangler/tmp/bundle-BhdYGt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-BhdYGt/middleware-loader.entry.ts
